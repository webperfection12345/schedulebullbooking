...
function wp_schedulebull_booking( $atts ) {
   ...
   $getApikey=get_option( 'schedule_bull_api_key' );
	...
	if(isset($_POST['bfsubmit'])){
	 ...
	 $outboundTime = $_POST['outbound_time'];
	 $Outbound=$_POST['outbound_date'].''.$outboundTime;
	 $fromPaSSenger=$_POST['fromPassenger'];
	 $returnTim = $_POST['return_time'];
	 $returnDate=$_POST['returndate'].''.$returnTim;
...
if(isset($_POST['ordersubmit'])){
	...
	$comment = 'Email:'.$customeremail.'| 2nd mobile:'.$moible_number2.'| Luggage info:'.$luggageinfo.'| Oversize luggage info:'.$oversizedluggage.'| Payment via Stripe:'.$paymentviastripe.'| Remaining balance:'.$payremainingbalance.'| Infant seats:'.$childseats.'| Child seats:'.$childseats2.'| Booster seats:'.$childseats3.'| Notes:'.$additionalnote;
...
for($havepassanger=1; $havepassanger <= $queuntity; $havepassanger++)
{
 ...
 $responsep = file_get_contents($palceorderapi);
  //echo " res2 <pre>";  print_r( $responsep);
$placeORDER[]=json_decode($responsep, true);
//echo "one buy <pre>";print_r($placeORDER);
    //if(!empty($placeORDER)){
    if($placeORDER[0]['error'] === 'Incorrect reservation time'){  
        $successmsg2Err="You're Booking hase not been accepted because some information is wrong, Please try again.";
    }else{
...
// echo " res <pre>"; print_r($responsep);
$placeORDER2[]=json_decode($responsep, true);
// print_r($placeORDER2);
    if($placeORDER2[0]['error'] === 'Incorrect reservation time'){
    //if(!empty($placeORDER2)){
        $successmsg2Err="You're Booking hase not been accepted because some information is wrong, Please try again.";
    }else{
        $successmsg2="Thank you for your order. Your reservation has been booked. Reservation number sent in your email Id.";
    }
...
//echo "<pre>";
//echo "<pre> placeORDER "; print_r($placeORDER);
//echo "<pre> placeORDER2 "; print_r($placeORDER2);
$pushorderid= array_merge((array)$placeORDER,(array)$placeORDER2);
//email function.
//print_r($pushorderid);
//echo 'end</pre>';

if($pushorderid[0]['error'] !== 'Incorrect reservation time'){
...
?>
<section class="modal-body" style="display:none;">
         <div class="section-form-main section-model"> 
	...
	<?php   } else{?>
                <form class="bookingFrom01" action="<?php echo get_permalink();?>?booking_form=true" method="post" autocomplete="off">
						 ...
						 <input type="hidden" name="u_varFromLocation" value="<?php echo $FromLocation;?>" class="">
						 <input type="hidden" name="u_varToLocation"   value="<?php echo $ToLocation;?>" class="">
						 <input type="hidden" name="u_dateOutbound"    value="<?php echo $Outbound;?>" class="">
						 <input type="hidden" name="u_returnDate"      value="<?php echo $returnDate;?>" class="">
						 <input type="hidden" name="u_fromPaSSenger"   value="<?php echo $fromPaSSenger;?>" class="">
						 <input type="hidden" name="u_TOPassenger" value="<?php echo $TOPassenger;?>" class="">
						 <input type="hidden" name="u_book_Price" value="<?php echo $book_Price;?>" class="">
						 <input type="hidden" name="u_reserve_Price" value="<?php echo $reserve_Price;?>" class="">
						 <input type="hidden" name="u_tototal_Price" value="<?php echo $TOtotal_Price;?>" class="">
						 
                  <div class="col d-flex justify-content-start border-btm align-items-center form-input">
                  ...
                  <div class="form-bg-color">
                     <div class="row">
						...
						<div class="d-flex align-items-center">
                                <div class="w-50 form-sub-heading">
                                    <p>Pick-up locaction</p>
                                </div>
                                <div class="w-50 form-sub-heading">
                                    <p>Drop-off locaction</p>
                                </div>
                            </div>
                           <div class="bg-white rounded d-flex  align-items-center py-2 mt-2">
                              ...
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                                 ...
                                 <select name="bfromlocation" class="form-select-1-a fromlocation" aria-label="Default select example" id="fromlocations" required>
                                    ...
                                    <?php foreach($getfromdata as $getfromdatas) { if(!empty($getfromdatas->name)){ ?>
				                      		<option data-from-id="<?php echo $getfromdatas->id;?>" <?php if($FromLocation == $getfromdatas->name){ echo 'selected="selected"'; } ?> value="<?php echo $getfromdatas->name;?>"><?php echo $getfromdatas->name;?></option>
			                           <?php } 
			                           } ?>
                                 </select>
                                 <i class="fa-solid fa-location-dot"></i>
                              ...
                              <div class="col d-flex  align-items-center form-input px-3" id="todatabyajax">
                                    ...
                                    <?php if($atts['fromlocation']  && $atts['tolocation'] ){?>
                                 <input name="btlocation" type="text" class="border-0 px-1 py-2 w-100 tolocation" id="tolocation"   placeholder="Transfer to" value="<?php if($ToLocation == $atts['tolocation']){ echo $ToLocation; }else{ echo $atts['tolocation']; } ?>" required>
                                  <i class="fa-solid fa-location-dot"></i>
                                 <?php }else {?>
                                 <select autofocus="autofocus" name="btlocation" class="form-select-1-a tolocation" id="tolocation" aria-label="Default select example" required>
                                    ...
                                    <option <?php if( $ToLocation ){ echo 'selected="selected"'; } ?>  value="<?php echo $ToLocation; ?>"><?php echo $ToLocation; ?></option>
                        ...
                        <div class="col-md-6 trensfer-form for-mobile">
						<div class="d-flex align-items-center">
                                <div class="w-66.66 form-sub-heading">
                                    <p>Pick-up date & time</p>
                                </div> 
                                <div class="w-33.33 form-sub-heading">
                                    <p>Passengers</p>
                                </div>
							</div>
                           <div class="bg-white rounded d-flex  align-items-center py-2 mt-2 outbond-one-p">
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                                 <input name="outbound_date" type="text" class="border-0 px-1 py-2 w-100 outbound-form Outbound1" id="datepicker" data-date="" data-date-format="j M Y" value=" <?php if($Outbound){ echo $Outbound; } ?>" placeholder="Pickup Date" required>
                                 <i class="fa-solid fa-calendar-days"></i>
                              </div>
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                        		<input name="outbound_time" class="border-0 px-1 py-2 w-100 outbound-form Outbound1" type="text" placeholder="Pick-up Time" id="timePicker" data-id="g:ia" value="<?php echo  date('G:i' ,strtotime($Outbound)); ?>" required>
                              ...
                              <select autofocus="autofocus" name="fromPassenger"  class="form-select-1-a fromsbblocation fromPassenger" id="fromPassenger" aria-label="Default select example" required>  
                                    ...
                                    <option <?php if($fromPaSSenger == 1){ echo 'selected="selected"'; } ?> value="1">1</option>
                                    <option <?php if($fromPaSSenger == 2){ echo 'selected="selected"'; } ?> value="2">2</option>
                                    <option <?php if($fromPaSSenger == 3){ echo 'selected="selected"'; } ?> value="3">3</option>
                                    <option <?php if($fromPaSSenger == 4){ echo 'selected="selected"'; } ?> value="4">4</option>
                                    <option <?php if($fromPaSSenger == 5){ echo 'selected="selected"'; } ?> value="5">5</option>
                                    <option <?php if($fromPaSSenger == 6){ echo 'selected="selected"'; } ?> value="6">6</option>
                                    <option <?php if($fromPaSSenger == 7){ echo 'selected="selected"'; } ?> value="7">7</option>
                                    <option <?php if($fromPaSSenger == 8){ echo 'selected="selected"'; } ?> value="8">8</option>
                                    <option <?php if($fromPaSSenger == 9){ echo 'selected="selected"'; } ?> value="9">9</option>
                                    <option <?php if($fromPaSSenger == 10){ echo 'selected="selected"'; } ?> value="10">10</option>
                                    <option <?php if($fromPaSSenger == 11){ echo 'selected="selected"'; } ?> value="11">11</option>
                                    <option <?php if($fromPaSSenger == 12){ echo 'selected="selected"'; } ?> value="12">12</option>
                                    <option <?php if($fromPaSSenger == 13){ echo 'selected="selected"'; } ?> value="13">13</option>
                                    <option <?php if($fromPaSSenger == 14){ echo 'selected="selected"'; } ?> value="14">14</option>
                                    <option <?php if($fromPaSSenger == 15){ echo 'selected="selected"'; } ?> value="15">15</option>
                                    <option <?php if($fromPaSSenger == 16){ echo 'selected="selected"'; } ?> value="16">16</option>
                                    <option <?php if($fromPaSSenger == 17){ echo 'selected="selected"'; } ?> value="17">17</option>
                                    <option <?php if($fromPaSSenger == 18){ echo 'selected="selected"'; } ?> value="18">18</option>
                                    <option <?php if($fromPaSSenger == 19){ echo 'selected="selected"'; } ?> value="19">19</option>
                                    <option <?php if($fromPaSSenger == 20){ echo 'selected="selected"'; } ?> value="20">20</option>
                                    <option <?php if($fromPaSSenger == 21){ echo 'selected="selected"'; } ?> value="21">21</option>
                                    <option <?php if($fromPaSSenger == 22){ echo 'selected="selected"'; } ?> value="22">22</option>
                                    <option <?php if($fromPaSSenger == 23){ echo 'selected="selected"'; } ?> value="23">23</option>
                                    <option <?php if($fromPaSSenger == 24){ echo 'selected="selected"'; } ?> value="24">24</option>
                                 </select>
                     ...
                     <div class="row py-3">
                        ...
                        <div class="col-md-6 return-fields trensfer-form">
                           ...
                           <div class="bg-white rounded d-flex  align-items-center py-2 mt-1">
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                                 <input name="returndate" type="text" class="border-0 px-1 py-2 w-100  outbound-form returnFORM" id="datepicker2" placeholder="Return Date" value="<?php if( $returnDate ){ echo $returnDate; } ?>" required>
                                 <i class="fa-solid fa-calendar-days"></i>
                                 
                              </div>
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                        		<input name="return_time" class="border-0 px-1 py-2 w-100  outbound-form returnFORM" type="text" placeholder="Return Time" id="timePicker2" value="<?php echo  date('G:i' ,strtotime($returnDate)); ?>" required>
                        		 <i class="fa-solid fa-clock"></i>
                        	  </div>
                              <div class="col d-flex  align-items-center form-input px-3">
                                 <select name="tOPassenger" class="form-select-1-a tosbblocation" id="Passengersto" aria-label="Default select example" required>
                                    <option value="">Passengers</option>
                                    <option <?php if($TOPassenger == 1){ echo 'selected="selected"'; } ?> value="1">1</option>
                                    <option <?php if($TOPassenger == 2){ echo 'selected="selected"'; } ?> value="2">2</option>
                                    <option <?php if($TOPassenger == 3){ echo 'selected="selected"'; } ?> value="3">3</option>
                                    <option <?php if($TOPassenger == 4){ echo 'selected="selected"'; } ?> value="4">4</option>
                                    <option <?php if($TOPassenger == 5){ echo 'selected="selected"'; } ?> value="5">5</option>
                                    <option <?php if($TOPassenger == 6){ echo 'selected="selected"'; } ?> value="6">6</option>
                                    <option <?php if($TOPassenger == 7){ echo 'selected="selected"'; } ?> value="7">7</option>
                                    <option <?php if($TOPassenger == 8){ echo 'selected="selected"'; } ?> value="8">8</option>
                                    <option <?php if($TOPassenger == 9){ echo 'selected="selected"'; } ?> value="9">9</option>
                                    <option <?php if($TOPassenger == 10){ echo 'selected="selected"'; } ?> value="10">10</option>
                                    <option <?php if($TOPassenger == 11){ echo 'selected="selected"'; } ?> value="11">11</option>
                                    <option <?php if($TOPassenger == 12){ echo 'selected="selected"'; } ?> value="12">12</option>
                                    <option <?php if($TOPassenger == 13){ echo 'selected="selected"'; } ?> value="13">13</option>
                                    <option <?php if($TOPassenger == 14){ echo 'selected="selected"'; } ?> value="14">14</option>
                                    <option <?php if($TOPassenger == 15){ echo 'selected="selected"'; } ?> value="15">15</option>
                                    <option <?php if($TOPassenger == 16){ echo 'selected="selected"'; } ?> value="16">16</option>
                                    <option <?php if($TOPassenger == 17){ echo 'selected="selected"'; } ?> value="17">17</option>
                                    <option <?php if($TOPassenger == 18){ echo 'selected="selected"'; } ?> value="18">18</option>
                                    <option <?php if($TOPassenger == 19){ echo 'selected="selected"'; } ?> value="19">19</option>
                                    <option <?php if($TOPassenger == 20){ echo 'selected="selected"'; } ?> value="20">20</option>
                                    <option <?php if($TOPassenger == 21){ echo 'selected="selected"'; } ?> value="21">21</option>
                                    <option <?php if($TOPassenger == 22){ echo 'selected="selected"'; } ?> value="22">22</option>
                                    <option <?php if($TOPassenger == 23){ echo 'selected="selected"'; } ?> value="23">23</option>
                                    <option <?php if($TOPassenger == 24){ echo 'selected="selected"'; } ?> value="24">24</option>
                                 </select>
                                 <i class="fa fa-users" aria-hidden="true"></i>
                              </div>
                           </div>
                        </div>
                        <?php $promo_val = get_option( 'check_promocode_on_off' ); 
                        if($promo_val == "checked" ){ ?>
						<div class="col-md-6 trensfer-form for-promo">
						<div class="w-100 form-sub-heading">
                                        <p>Promo code</p>
                        </div>
                        ...
                        <div class="col-md-12 mt-2">
                           <div class="row d-flex justify-content-center  align-items-center">
						    <div class="col-md-12 col-sm-12 mt-3 d-flex justify-content-center  align-items-center form-input">
                                <div class="col-md-6  d-flex justify-content-center  align-items-center form-input px-2 text-center">
                                 <p class="text-white  d-flex justify-content-center  align-items-center price-for-private">  <strong>Price for Private transfer: <span class="text-danger form-price bookprice">
                                 	<?php if($book_Price){ echo $book_Price; }else{ echo "€0.00"; } ?>
                                 </span> </strong></p>
                              </div>
                             </div>
                              <div class="col mt-3 d-flex justify-content-center  align-items-center form-input">
                                 <input type="submit" name="bfsubmit" class="book-now-button" value="Update Booking">
                              </div>
                           </div>
                        </div>
                     </div>
               ...
               <?php } ?>
                <a class="cancel"><i class="fa-solid fa-circle-xmark"></i></a>
            </div>
...
<section class=" mt-5 py-5 main-form-booked">
  	<div class="container">
	    ...
	    <?php }?>
	    <?php 	if(!empty($successmsg2Err)){?>
	        <div class="alert alert-danger" role="alert">
                <?php echo $successmsg2Err;?>
            </div>	
	    <?php }?>
 ...
 <div class="row">
			<div class="update-bookings d-flex justify-content-start align-items-center">
 			<h4 class="Transfer-edit">Transfer:</h4><a type="button" href="#" class="booking-edit btn btn-primary px-4">Edit</a><?php // echo get_the_permalink(get_the_ID());?>
 		</div>
 	<!-- second form -->
 ...
 <input type="hidden" name="tototal_Price" value="<?php echo $TOtotal_Price;?>" class="">
	...
	<optgroup label="Other countries">
       ...
       <div class="col-6 second-form-inpu"> 
       	<div class="text-center p-0 my-3 form-user-name">
       	    <input type="hidden" id="c_name_mb" value="">
       	     <input type="hidden" id="c_code_mb" value="">
       	    <input type="hidden" id="mobile" value="">
       		 <input type="number" name="mobile_phone" class="check_code form-control rounded-0"  placeholder="Mobile phone:*" required>
    ...
    <div class="row details-passenger">
 	...
 	<div class="row details-passenger hideremainpay" style="display:none">
       	<div class="col-12 mt-4">
 			...
 			<div class="col-6">
       		<div class="p-0  my-3">
         	<input class="remaining_blance" type="radio" name="remaining_blance" id="flexRadioDefault4" value="We will send payment link 3 days before transfer date." >
       	...
       	<div class="col-12 mt-4">
 			...
 			<div class="col-6 second-form-inpu">
       	   ...
       	   <div class="col-6 second-form-inpu">
				 ...
				 </p><span class="nb_inValid" style="color:red;"></span>
				 <input type="submit" name="ordersubmit" <?php if($book_Price == '€0.00'){ echo "disabled";}?> value="Order Now" class="order_btmn btn btn-danger">
               </div>
       	   </div>
 	</div>
</form>
                  ...
                  <div class="form-bg-color">
                     <div class="row">
						...
						<div class="d-flex align-items-center">
                            ...
                            <?php// echo "<pre> 2113 "; print_r($getfromdata); ?> 
                           <div class="bg-white rounded d-flex  align-items-center py-2 mt-2">
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                                 <?php if($atts['fromlocation']  && $atts['tolocation'] ){?>
                                 <input name="bfromlocation" type="text" class="border-0 px-1 py-2 w-100 fromlocation" id="fromlocations"   placeholder="Transfer from" value="<?php echo $atts['fromlocation']; ?>" required>
						...
						<div class="d-flex align-items-center">
                           ...
                           <div class="bg-white rounded d-flex  align-items-center py-2 mt-2 outbond-one-p">
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                                 <input name="outbound_date" type="text" class="border-0 px-1 py-2 w-100 outbound-form Outbound1" id="datepicker" data-date="" data-date-format="j M Y g:ia"  placeholder="Pickup Date" required>
                                 <i class="fa-solid fa-calendar-days"></i>
                              </div>
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                        		<input name="outbound_time" class="border-0 px-1 py-2 w-100 outbound-form Outbound1" type="text" placeholder="Pick-up Time" id="timePicker" data-id="g:ia" required>
                        	<i class="fa-solid fa-clock"></i>
                        	  </div>
                              <div class="col d-flex  align-items-center form-input px-3">
                              <select autofocus="autofocus" name="fromPassenger"  class="form-select-1-a fromsbblocation fromPassenger" id="fromPassenger" aria-label="Default select example" required>
                     ...
                     <div class="row py-3">
						...
						<div class="d-flex align-items-center">
                           ...
                           <div class="bg-white rounded d-flex  align-items-center py-2 mt-1">
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                                 <input name="returndate" type="text" class="border-0 px-1 py-2 w-100  outbound-form returnFORM" id="datepicker2" placeholder="Return Date" required>
                                 <i class="fa-solid fa-calendar-days"></i>
                                 
                              </div>
                              <div class="col d-flex  align-items-center form-input px-2 border-input">
                        		<input name="return_time" class="border-0 px-1 py-2 w-100  outbound-form returnFORM" type="text" placeholder="Return Time" id="timePicker2" required>
                        		<i class="fa-solid fa-clock"></i>
                        	  </div>
                              <div class="col d-flex  align-items-center form-input px-3">
                                 <select name="tOPassenger" class="form-select-1-a tosbblocation" id="Passengersto" aria-label="Default select example" required>
...
function isValidNumber(number, country) {
    var country = document.getElementById('c_name_mb').value; 
   console.log(country)
  // libphonenumber.parsePhoneNumber('9098765432', 'IN')
   return new libphonenumber.parsePhoneNumber(number, country).isValid()
}
...
$("#datepicker").flatpickr({
    ...
    onClose: function(selectedDates, dateStr, instance){
        ...
        //jQuery('#tolocation option:selected').prop('disabled', true);
             var curtime =jQuery('#timePicker').val();
    var curDate1 =$('#datepicker').val()+' '+curtime;
...
$("#timePicker").flatpickr({
    ...
    onClose: function(selectedDates, dateStr, instance){
    	 ...
    	 var c_time = jQuery('#timePicker').val();
        var curtime =curDate1+' '+c_time;
        var fromlocationName= jQuery('#fromlocations').val();
        var tolocationName= jQuery('#tolocation').val();
        var frompassengeR= jQuery('#fromPassenger').val();
       
         if(frompassengeR && curDate1){
            jQuery.ajax({
                type : "post",
                dataType : "html",
                url : "<?php echo admin_url('admin-ajax.php'); ?>",
                data : {action: "get_price_by_fromdate", from_name : fromlocationName,to_name : tolocationName,frompass : frompassengeR, fromDate : curtime},
        		beforeSend: function() {
                    $('#loader').show();
                },
                complete: function(){
...
var FLATPICKER_RITORNO=flatpickr('#datepicker2',{
   ...
   //   alert('h22222222222222');
	...
	onClose: function(selectedDates, dateStr, instance){
       ...
       var fromlocationName= jQuery('#fromlocations').val();
var tolocationName= jQuery('#tolocation').val();
 var returntime= jQuery('#timePicker2').val();
var returnDate= jQuery('#datepicker2').val()+' '+returntime;
...
var FLATPICKER_RITORNO=flatpickr('#timePicker2',{
    time_24hr: true,
	disableMobile: "true", 
	enableTime: true,
	noCalendar: true,
    ...
    onClose: function(selectedDates, dateStr, instance){
        ...
        var returntime1= jQuery('#timePicker2').val();
        var returntime= returnDate+' '+returntime1;
        var topassengeR= jQuery('#Passengersto').val();
        if(topassengeR){
            jQuery.ajax({
                 type : "post",
                 dataType : "html",
                 url : "<?php echo admin_url('admin-ajax.php'); ?>",
                 data : {action: "get_price_by_todate", from_name : fromlocationName,to_name : tolocationName,return_time : returntime,to_passengeR : topassengeR},
        		beforeSend: function() {
                 $('#loader').show();
                },
                complete: function(){
...
var fromoutbondtime= jQuery('#timePicker').val();
var frompassengeR= jQuery('#fromPassenger').val();
   jQuery.ajax({
         type : "post",
         dataType : "html",
         url : "<?php echo admin_url('admin-ajax.php'); ?>",
         data : {action: "get_booking_price", from_name : fromlocationName,to_name : tolocationName,outbounddate : fromoutbondDate, outboundtime: fromoutbondtime,  from_passengeR : frompassengeR},
		 beforeSend: function() {
         $('#loader').show();
          },
         complete: function(){
...
var hiddenfromprice =jQuery('#schedule_book_price').val();
   jQuery.ajax({
         ...
         dataType : "html",
         url : "<?php echo admin_url('admin-ajax.php'); ?>",
         data : {action: "bookingprice_return", from_name : fromlocationName,to_name : tolocationName, return_Date : returnDate, return_time : returntime, to_passengeR : topassengeR,hidden_fromprice : hiddenfromprice},
		 beforeSend: function() {
         $('#loader').show();
          },
         complete: function(){
...
var tolocationName= jQuery('#tolocation').val();
var fromoutbondtime= jQuery('#timePicker').val();
var fromoutbondDate= jQuery('#datepicker').val()+' '+fromoutbondtime;
//console.log(fromoutbondDate);
var TORETuRNtime=  jQuery('#timePicker2').val();
var TORETuRNDate= jQuery('#datepicker2').val()+' '+TORETuRNtime; 
var frompassengeR= jQuery('#fromPassenger').val();
var schedulebookprice= jQuery('#schedule_book_price').val();
console.log(schedulebookprice);
//if(PromroCode !=''){
  jQuery.ajax({
         type : "post",
         dataType : "html",
...
$(document).ready(function() {
  ...
  //  
    $('select[name="dialing_code"]').on('change', function() {
       //  alert( this.value );
    //   $(this).find('option').attr("data-countrycode").val();
        var c_n = $('option:selected', this).attr('data-countrycode');
        $('#c_name_mb').val(c_n); 
       
        var c_code = this.value; 
        $('#c_code_mb').val(c_code); 
    });
 
    $('.check_code').on('keyup keypress blur ', function() {
   //  alert( this.value );
    
     var movile_n =  $(this).val();
        $('#mobile').val(movile_n);
        
    });

 //  $('.order_btmn').click(function(){
         
    $('.check_code').blur(function(){
         var c_number = document.getElementById('c_code_mb').value;
         var m_number = document.getElementById('mobile').value;
         var number = c_number+''+m_number;
        // console.log(number);
        if(number.length <4) {
         alert('Too Short ');
         $('.order_btmn').attr('disabled' , true);
         $('.nb_inValid').text("Please fill Valid Mobile Number");
        } else if(isValidNumber(number)) {
           $('.order_btmn').attr('disabled' , false);
            $('.nb_inValid').hide();
        //  alert('Valid number')
        } else {
	...
	$('.booking-edit').click(function(){
        $('.section-model').addClass('active-modal');
         $('.modal-body').addClass('modal');
        $('.modal-body').show(); 
        $('.section-model.active-modal').show(); 
        jQuery('.cancel').show(); 
    });
    jQuery('.cancel').click(function(){
		jQuery('.section-model').hide(); 
		jQuery('.cancel').hide(); 
		  jQuery('.modal-body').removeClass('modal');
		jQuery('.section-model').removeClass('active-modal');
	}); 
